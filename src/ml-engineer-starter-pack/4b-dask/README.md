...
```python
df = dd.read_csv("../data/train.Video_Games_Sales_as_at_22_Dec_2016.csv")
df.head()
df = dd.read_csv("../data/train.Video_Games_Sales_as_at_22_Dec_2016.csv", dtype={"User_Score": "object", "Year_of_Release": "float64"})
df.head()
df.groupby("Genre").size()
task = df.groupby("Genre").size()
type(task)
task.compute()
```
